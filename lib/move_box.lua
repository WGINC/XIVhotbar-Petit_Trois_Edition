...
function move_boxes:check_slot(x, y)
  ...
  local dest_row = 0
  local dest_col = 0
  local w = icon_lib:get_width()
  local h = icon_lib:get_height()
  for i = 1, self.theme.rows, 1 do
    for j = 1, self.theme.columns, 1 do
      local pos_x = icon_lib:get_slot_x(i, j)
      local pos_y = icon_lib:get_slot_y(i, j)
      local off_x = pos_x + w
      local off_y = pos_y + h
...
function move_boxes:move_hotbars(type, x, y, delta, blocked)
  ...
  elseif type == 2 then -- Mouse left release
    ...
    if (self.moved_box_info.slot_active == true and row_clicked ~= nil and slot_clicked ~= nil) then
      ...
      -- Reset to the slot's current correct position (accounts for any row that was moved this session)
      box_config.icons[row].slot[col]:pos(icon_lib:get_slot_x(row, col), icon_lib:get_slot_y(row, col))
  ...
  elseif type == 0 then -- Mouse move
    ...
    elseif (self.moved_box_info.row_active == true) then
      ...
      -- Keep theme.offsets in sync so icon_lib returns correct live positions,
      -- and move the slot overlay images to match the dragged row.
      local r = self.moved_box_info.box_index
      self.theme.offsets[tostring(r)].OffsetX = new_x
      self.theme.offsets[tostring(r)].OffsetY = new_y
      for s = 1, self.theme.columns do
        box_config.icons[r].slot[s]:pos(icon_lib:get_slot_x(r, s), icon_lib:get_slot_y(r, s))
      end
    end
  end
